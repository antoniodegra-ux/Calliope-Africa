...
#        if ctryf_writeout: all_table_profiles.to_csv(out_dir +'/'+saveas + '_profiles.csv')
        ...
        if plot_count == 1:
           axs = np.expand_dims(axs, axis=1)  # axs[0] → axs[0,0], axs[1] → axs[1,0], ecc.
...
# AGGIUNTA: salva tabella zone - cluster (cluster parte da 1)
zones_cluster_map = all_table_all[["cluster"]].copy()
zones_cluster_map["cluster"] = zones_cluster_map["cluster"] + 1
zones_cluster_map["CtryName"] = all_table_all["CtryName"] if "CtryName" in all_table_all.columns else None
zones_cluster_map = zones_cluster_map.reset_index().rename(columns={"index":"Zone_ID"})
zones_cluster_map = zones_cluster_map[["CtryName","Zone_ID","cluster"]]
zones_cluster_map.to_csv("/Users/antoniodegrazia/Desktop/Model-Supply-Regions-MSR-Toolset-main 2/5. Clustering/results/zones_cluster_assignment.csv", index=False)
...
bigT = bigT[cols]
bigT.to_csv('/Users/antoniodegrazia/Desktop/Model-Supply-Regions-MSR-Toolset-main 2/5. Clustering/results/bigT.csv')
all_table_all.to_csv('/Users/antoniodegrazia/Desktop/Model-Supply-Regions-MSR-Toolset-main 2/5. Clustering/results/unclustered_zones_all.csv')
