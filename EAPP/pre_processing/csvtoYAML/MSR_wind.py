...
input_file = "/Users/antoniodegrazia/Downloads/bigT_wind_finito.csv"  # Replace with your actual file name

# Read the entire input file
df = pd.read_csv(input_file)

# Create the output folder
output_folder = "/Users/antoniodegrazia/Desktop/tesiCigala_DeGrazia/clustering/pre/csvtoYAML/YAML_output_finale_wind"
...
def create_yaml_tech(row, index):
    entry = {
        f"Wind_{country_code}_MSR{row['Zone_ID']}": {  # Use MSR index starting from 1 for naming
            "essentials": {
                "color": "#E32E18",
                "name": f"Wind Power Plant {country_code} {row['Zone_ID']}",
                "parent": "supply_plus",
            ...
            "costs": {
                "monetary": {
                    "energy_cap": 1120 + row['trCAPEX-kW'],
                    "om_annual": 35,  # Can be adjusted based on your data
...
def create_yaml_location(row, country_code, timeseries_name):
    location = {
        f"Wind_{country_code}_MSR{row['Zone_ID']}": {
            "constraints": {
                "energy_cap_min": 0,
