# -*- coding: utf-8 -*-
"""
Autarky 2030 — 6 PNG separati: pie distribuzione investimenti (B$) per Stato
...
- Ogni PNG è una torta con la ripartizione per Paese (somma OCGT+Wind+PV+Storage in B$).
"""
...
OUT_DIR  = Path(r"C:\Users\ilari\OneDrive\Desktop\tesi\GRAFICI\INV BY COUNTRY\PIES_noregioni2030")
OUT_DIR.mkdir(parents=True, exist_ok=True)

DEBUG = True
MIN_SHARE_LABEL = 0.01  # 1% -> paesi sotto questa quota finiscono in "Other"
...
def get_country_from_row(row) -> str:
    """Ordine: locs → techs (PV_/Wind_) → 'UNK'."""
    loc = str(row.get("locs", "")).strip()
    if loc and loc.lower() != "nan":
        tok = loc.split("_")[0]  # es. 'DRC_E' -> 'DRC'
        if tok:
            return tok.upper()
...
def aggregate_investments_by_country(df: pd.DataFrame) -> pd.DataFrame:
    """
    Ritorna: country, Total_BUSD (somma OCGT+Wind+PV+Storage in B$)
    ...
    if df.empty or "techs" not in df.columns or "cost_investment" not in df.columns:
        return pd.DataFrame(columns=["country","Total_BUSD"])
    ...
    sub = df.loc[use_mask, ["country","cost_investment"]].copy()

    tot = (sub.groupby("country", as_index=False)["cost_investment"].sum()
...
def collect_scenario_totals(base_dir: Path, year: str, scen_folder: str) -> pd.DataFrame:
    ...
    return aggregate_investments_by_country(df_cost)

def build_color_map(dict_dfs: dict) -> dict:
    """Colori coerenti per Paese su tutti gli scenari (tab20 ciclico)."""
    all_countries = sorted(set(
        c for df in dict_dfs.values() for c in (df["country"].tolist() if not df.empty else [])
    ))
    palette = plt.cm.tab20(np.linspace(0, 1, 20))
    return {c: palette[i % 20] for i, c in enumerate(all_countries)} | {"Other": (0.85,0.85,0.85,1.0)}

def save_pie_for_scenario(df: pd.DataFrame, scen: str, color_map: dict, out_dir: Path):
    """Salva un PNG con la torta per lo scenario indicato."""
    ...
    if not minor.empty:
        ...
        major = pd.concat([major, pd.DataFrame([{"country":"Other","Total_BUSD":other_val,"share":other_val/total}])],
                          ignore_index=True)
    major = major.sort_values("Total_BUSD", ascending=False)

    labels = major["country"].tolist()
    sizes  = major["Total_BUSD"].to_numpy()
    colors = [color_map.get(c, color_map["Other"]) for c in labels]
    ...
    # Legenda a destra
    ax.legend(wedges, labels, title="Country", loc="center left",
...
# Calcola i totali per Paese per ciascuno scenario
data = {scen: collect_scenario_totals(BASE_2030, year, scen) for scen in scenarios}

# Colori coerenti tra tutti i grafici
cm = build_color_map(data)

# Salva 6 PNG separati
