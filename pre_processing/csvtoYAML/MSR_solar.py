...
input_file = "/Users/antoniodegrazia/Desktop/Model-Supply-Regions-MSR-Toolset-main 2/5. Clustering/results/bigT.csv"  # Replace with your actual file name

# Read the entire input file
df = pd.read_csv(input_file)

# Create the output folder
output_folder = "/Users/antoniodegrazia/Desktop/tesiCigala_DeGrazia/clustering/pre/csvtoYAML/YAML_output_finale"
...
yaml_tech_file = os.path.join(output_folder, "Solar_Tech.yaml")

# Unified timeseries output file
timeseries_name = "Solar_WAPP_MSR.csv"
...
# Function to create a dictionary representing each solar plant in the required format
def create_yaml_tech(row, country_code):
    entry = {
        f"PV_{country_code}_MSR{row['Zone_ID']}": {
            "essentials": {
                "color": "#FFA52B",
                "name": f"PV Power Plant {country_code} {row['Zone_ID']}",
                "parent": "supply_plus",
            ...
            "costs": {
                "monetary": {
                    "energy_cap": 1200 + row['trCAPEX-kW'],
                    "om_annual": 15,
                    "interest_rate": 0.10
                },
                "co2": {
...
def create_yaml_location(row, country_code, timeseries_name):
    location = {
        f"PV_{country_code}_MSR{row['Zone_ID']}": {
            "constraints": {
                "energy_cap_min": 0,
                "energy_cap_max": row['Max Capacity MW'] * 1000,  # Convert MW to kW
                "resource": f"file={timeseries_name}:{country_code}{row['Zone_ID']}",
...
def append_timeseries_to_file(df, country_code, file_path):
    ...
    for idx, row in df.iterrows():
        header = f"{country_code}{row['Zone_ID']}"  # Format: CountryCode_MSRID
...
for country_name, country_df in df.groupby("CtryName"):
    ...
    yaml_location_file = os.path.join(output_folder, f"Solar_{country_name}_location.yaml")
